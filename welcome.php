 <?php 
    ...
    //Recuperare solante un elemnto, quindi verifico che sia diverso da questo.
    //Quindi dfinisco array clone delle piante

    $filteredArry = $plants;

    //-------------Ricerca filtro PER NOME
    
    // Se esiste la variabile $_GET ['name'] ed è diversa dalla stringa vuota
    if(isset($_GET['name']) && $_GET['name'] != '' ){

        //Prima definisco un array temporaneo
        $tempArray = [];       

        //Ciclo l'array filtered
        foreach($filteredArry as $item){ 
           // controllo che $item['name'] sia uguale a quello che ho pasato ed è contenuto in $_GET['name']

             if($item['name'] == $_GET['name']){
              $tempArray[] = $item; // questa sintassi vol dire che sto pushando un elemnto dentro l'array
   
             }          

        }
        //riassegno a filtereArray il nuovo array con gli elementi effettivi filtrati

        $filteredArry = $tempArray;      
    }
    //-----------------------Ricerca filtro per FAMIGLIA 

    // Se esiste la variabile $_GET ['name'] ed è diversa dalla stringa vuota
    if(isset($_GET['famiglia']) && $_GET['famiglia'] != '' ){
        ...
        //Ciclo l'array filtered
        foreach($filteredArry as $item){ 
           // controllo che $item['famiglia'] sia uguale a quello che ho pasato ed è contenuto in $_GET['famiglia']

             if($item['famiglia'] == $_GET['famiglia']){
              $tempArray[] = $item; // questa sintassi vol dire che sto pushando un elemnto dentro l'array
    ...
    //-----------------------Ricerca filtro per COMMESTIBILI

     // Se esiste la variabile $_GET ['name'] ed è diversa dalla stringa vuota
    if(isset($_GET['commestibili']) && $_GET['commestibili'] != '' ){
        ...
        //Ciclo l'array filtered
        foreach($filteredArry as $item){ 
           // controllo che $item['commestibili'] sia uguale a quello che ho pasato ed è contenuto in $_GET['commestibili']

             if($item['commestibili'] == $_GET['commestibili']){
              $tempArray[] = $item; // questa sintassi vol dire che sto pushando un elemnto dentro l'array
